...
class LidarSubsciber : public rclcpp::Node
{
  ...
  private:
    ...
    void laser_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
      ...
      len = (len > MAX_SAMPLE ? MAX_SAMPLE : len);
      for(int i = 0; i < len; i++){
    ...
    void point_callback(const sensor_msgs::msg::PointCloud::SharedPtr msg)
    {
      ...
      for (int i_pc = 0; i_pc < n_smp; i_pc++)
      {
        ...
        ranges[i_pc] += noise;
        cloud_b->at(i_pc).x = ranges[i_pc] * cos(angle_min + angle_inc * i_pc);
        cloud_b->at(i_pc).y = ranges[i_pc] * sin(angle_min + angle_inc * i_pc);
        cloud_b->at(i_pc).z = 0;
      }

      // TODO 3. pointcloud visualization
      pcl_viewer_->addPointCloud<pcl::PointXYZ>(cloud_b, "cloud_pc1");
      pcl_viewer_->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 6, "cloud_pc1");
      pcl_viewer_->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_COLOR , 0.00, 1.00, 1.00, "cloud_pc1");
      ...
      pcl_viewer_->removePointCloud("cloud_pc1");
    }
